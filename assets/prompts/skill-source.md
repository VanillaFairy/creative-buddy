...
description: Use when growing, querying or maintaining a knowledge graph — a vault folder whose hub note carries a "## Charter" block — or when starting a new one from a bare topic. Triggers on "grill me about X", "let's work on <graph>", "start a graph about X", asking a question about a graph's subject, correcting or retracting something recorded in one, working inside a graph folder, or "obsidian:knowledge-graph".
---
...
folders make this easy to do by accident, which is why `graph_check.py` now
refuses it.
...
| Anything outside the graph folder | ask first |
...
Append one file to `Log/`, named `Log/YYYY-MM-DD-<letter>.md` — `a` for the
first session that day, `b` for the next, the letter carrying on from the last
file present whether or not today has one. Two or three sentences: what was
established, where the thread stopped, any door the user closed, and anything
you took out as your own invention. The log is a bookmark, not the map:
continuity beyond it lives in the hub's Shape, which you refresh in the same
breath. Then run the checks.

## Session end, mechanically

After the log, run the checks over the graph folder:

    python scripts/graph_check.py <graph>
    python scripts/obligations.py <graph> --today YYYY-MM-DD
    python scripts/graph_check.py <graph> --tree

Silence from the first is a pass. Anything it prints, fix before you stop — an
unresolved parent usually means a title lost a character to the filesystem.
Anything `obligations` prints that surprises you means a grade landed wrong:
a deadline the user never set, or one they did set sitting in prose where the
register cannot see it. Go and look.

`--tree` draws the shape and tells you how many nodes hang off the hub. Read it
as a picture: if a dozen siblings sit at the top with nothing gathering them,
the graph has gone flat and the next session should start by proposing the
middle nodes that are missing.
