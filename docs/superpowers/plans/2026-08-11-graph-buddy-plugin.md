...
3. **Sort order is Windows truth.** The oracle runs on Windows where `str(Path)` uses `\` and `os.path.normcase` lowercases. TS sorts note lists with `sortKeyWindows(path) = path.replaceAll("/", "\\")`, and all case-insensitive comparisons use `casefold(s) = s.toLowerCase()`. String comparisons that feed oracle-visible ordering go through `comparePyStrings` (code-POINT order, like Python; JS `<` is UTF-16 code-unit order and diverges on astral chars). Fixture names avoid `ß` and other locale-sensitive casing.
...
5. **Graph list ordering:** Python `sorted(list[Path])` on Windows compares the CASEFOLDED segment tuples (`_parts_normcase`) — verified against Python 3.14: `Path("alpha") < Path("Beta")`. TS `comparePathSegments` compares `casefold`ed segments via `comparePyStrings`, returning 0 on fold-equal input (stable sort; NTFS forbids case-only siblings). [M1-review fix]
...
10a. **`str.strip()`, not `trim()`, is the frontmatter-delimiter rule.** JS `trim()` removes U+FEFF; Python's `strip()` does not (BOM is category Cf), which is load-bearing for locked decision 1. As built (hardening + M1 review): `frontmatter.ts` owns a `pyStrip` helper implementing Python's exact whitespace set (incl. `\x1c–\x1f`, excl. FEFF) and a shared closer-scan used by BOTH `parseFrontmatter` and the exported `stripFrontmatterBlock`; `notes.ts` imports the latter (its Task 5 snippet's private helper is superseded). The Task 4 snippet's `.trim()` calls are likewise superseded by `pyStrip`.
